...
class RoleTitleService(BaseService):
        ...
        if not req_data.id:
            ...
            if req_data.is_active and exits_rt.department.is_active is False:
                raise CustomException(http_code=400,
                                      code=error_code.ERROR_197_CAN_NOT_ACTIVE_ROLE_TILE_IN_DEPARTMENT_INACTIVE,
    ...
    def check_role_in_VNNG(self, role_title_id: int) -> bool:
        role_title = db.session.query(self.model).filter(self.model.id == role_title_id).first()
        if SalesRoleName.has_value(role_title.role_title_name):
            return True
        return False

    def get_role_name(self, role_title_id: int) -> str:
        role_title = db.session.query(self.model).filter(self.model.id == role_title_id).first()
        if role_title:
            return role_title.role_title_name

    def get_id_role_in_iam(self, role_title_id: int) -> int:
        role_name = self.get_role_name(role_title_id=role_title_id)
...
role_title_service = RoleTitleService()
