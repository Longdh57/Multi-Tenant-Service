...
from app.models import StaffTeam, Company, Team, Staff, DepartmentStaff, Department, Corporation
from app.schemas.sche_staff import StaffList, StaffRequest
from app.schemas.sche_team import TeamCreateRequest, TeamDetailResponse, TeamItemResponse, TeamListRequest, \
    TeamUpdateRequest
...
class TeamService(BaseService):
    ...
    def get_list_with_paging(self, team_list_req: TeamListRequest) -> Page[TeamItemResponse]:
        if team_list_req.company_id:
            ...
            companies_id = [team_list_req.company_id]
        if team_list_req.corporation_id:
            cor = db.session.query(Corporation).filter(Corporation.id == team_list_req.corporation_id).first()
            if not cor:
                raise CustomException(http_code=400, code='001', message="Corporation not found")
            companies = db.session.query(Company).filter(Company.corporation_id == team_list_req.corporation_id).all()
            companies_id = [company.id for company in companies]
        _query = db.session.query(self.model).filter(
            self.model.company_id.in_(companies_id), self.model.is_active)
    ...
    @staticmethod
    def make_query_get_staffs_in_teams(teams_id):
        return db.session.query(Staff).join(StaffTeam). \
            filter(Staff.id == StaffTeam.staff_id). \
            filter(StaffTeam.team_id.in_(teams_id)). \
